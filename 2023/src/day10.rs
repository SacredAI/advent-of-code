...
impl Day10 {
    ...
    fn can_move(tile: char, direction: Direction) -> bool {
        match direction {
            Direction::North => "|F7S".find(tile).is_some(),
            Direction::South => "|LJS".find(tile).is_some(),
            Direction::East => "-J7S".find(tile).is_some(),
            Direction::West => "-FLS".find(tile).is_some(),
        }
    }
                ...
                match next_dir {
                    Direction::South => {
                        if c_point.x < self.x_limit
                            && Day10::can_move(
                                self.get_tile(c_point.south_move()),
                                Direction::South,
                            )
                        {
                    ...
                    Direction::North => {
                        if c_point.x > 0
                            && Day10::can_move(
                                self.get_tile(c_point.north_move()),
                                Direction::North,
                            )
                        {
                    ...
                    Direction::West => {
                        if c_point.y > 0
                            && Day10::can_move(self.get_tile(c_point.west_move()), Direction::West)
                        {
                    ...
                    Direction::East => {
                        if c_point.y < self.y_limit
                            && Day10::can_move(self.get_tile(c_point.east_move()), Direction::East)
                        {
...
// #[aoc_generator(day9)]
pub fn input_generator(input: &str) -> Day10 {
    Day10::new(input)
}

#[aoc(day10, part1)]
pub fn part1(input: &str) -> u64 {
    let day = input_generator(input);
    let starting_location = day
    ...
    let (south_path, _) = day.traverse(south_start, Direction::North);
    if max_visited < south_path.len() {
        max_visited = south_path.len();
    }
    let (north_paths, _) = day.traverse(north_start, Direction::South);
    if max_visited < north_paths.len() {
        max_visited = north_paths.len();
    }
    let (west_paths, _) = day.traverse(west_start, Direction::West);
    if max_visited < west_paths.len() {
        max_visited = west_paths.len();
    }
    let (east_paths, _) = day.traverse(east_start, Direction::West);
...
pub fn part2(input: &str) -> u64 {
    ...
    let starting_location = day
        ...
        .expect("Starting position not provided");
    let south_start = starting_location.south_move();
    let (mut path, edges) = day.traverse(south_start, Direction::North);
    let mut edges = edges
        .into_iter()
    ...
    let points = iproduct!(0..day.x_limit, 0..day.y_limit);
    points
        .par_bridge()
        .filter(|&(x, y)| {
            let point = Point { x, y };
            !path.contains(&point) && day.check_point_inside_path(point, &edges_start)
        })
        .count() as u64
}
